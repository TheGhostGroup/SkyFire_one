...
void BattlegroundEY::CheckSomeoneJoinedPoint()
{
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            {
                Player* player = sObjectMgr->GetPlayer(m_PlayersNearPoint[EY_POINTS_MAX][j]);
                if (!player)
                {
...
void BattlegroundEY::CheckSomeoneLeftPoint()
{
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            {
                Player* player = sObjectMgr->GetPlayer(m_PlayersNearPoint[i][j]);
                if (!player)
                {
...
void BattlegroundEY::UpdatePointStatuses()
{
    for (uint8 point = 0; point < EY_POINTS_MAX; ++point)
    {
        ...
        {
            Player* player = sObjectMgr->GetPlayer(m_PlayersNearPoint[point][i]);
            if (player)
            {
...
void BattlegroundEY::RemovePlayer(Player* player, uint64 guid)
{
