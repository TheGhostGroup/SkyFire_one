...
bool OutdoorPvPSI::HandleAreaTrigger(Player* player, uint32 trigger)
{
    ...
    case SI_AREATRIGGER_A:
        if (player->GetTeam() == ALLIANCE && player->HasAura(SI_SILITHYST_FLAG, 0))
        {
            // remove aura
            ...
            player->ModifyFactionReputation(609, 20);
            // complete quest
            player->KilledMonsterCredit(SI_TURNIN_QUEST_CM_A, 0);
        }
        return true;
    case SI_AREATRIGGER_H:
        if (player->GetTeam() == HORDE && player->HasAura(SI_SILITHYST_FLAG, 0))
        {
            // remove aura
            player->RemoveAurasDueToSpell(SI_SILITHYST_FLAG);
            ++ m_Gathered_H;
