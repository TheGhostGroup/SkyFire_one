...
int32 OPvPCapturePointZM_GraveYard::HandleOpenGo(Player* player, uint64 guid)
{
    ...
    if (retval >= 0)
    {
        if (player->HasAura(ZM_BATTLE_STANDARD_A, 0) && m_GraveYardState != ZM_GRAVEYARD_A)
        {
        ...
        else if (player->HasAura(ZM_BATTLE_STANDARD_H, 0) && m_GraveYardState != ZM_GRAVEYARD_H)
        {
...
bool OPvPCapturePointZM_GraveYard::CanTalkTo(Player* player, Creature * c, GossipMenuItems /*gso*/)
{
