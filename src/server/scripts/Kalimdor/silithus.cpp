...
struct npc_anachronos_the_ancientAI : public ScriptedAI
{
    ...
    void HandleAnimation()
    {
        ...
        Unit* Fandral = player->FindNearestCreature(C_FANDRAL_STAGHELM, 100, me);
        Unit* Arygos = player->FindNearestCreature(C_ARYGOS, 100, me);
        Unit* Caelestrasz = player->FindNearestCreature(C_CAELESTRASZ, 100, me);
        Unit* Merithra = player->FindNearestCreature(C_MERITHRA, 100, me);
        ...
        if (eventEnd == false)
        {
            switch (AnimationCount)
            {
                ...
                case 51:
                {
                    ...
                    for (uint8 i = 0; i < 4; i++)
                    {
                        mob = player->FindNearestCreature(entries[i],300, true);
                        loopbreaker = 0;
                        ...
                        mob = player->FindNearestCreature(entries[i],300, false);
                        loopbreaker = 0;
                        while (mob != NULL && loopbreaker < 67)
                        {
                            mob->RemoveFromWorld();
